...
app=FastAPI(title="My AI API",version="9.0")
...
async def real_answer(message,history,memory):
 from openai import OpenAI
 memory_text="\n".join("- "+x for x in memory)
 instructions=SYSTEM_PROMPT
 if memory_text: instructions += "\n\nLong-term user memory (use only when relevant):\n"+memory_text
 items=[{"role":r,"content":c} for r,c in history[-12:]]
 items.append({"role":"user","content":message})
 return OpenAI(api_key=API_KEY).responses.create(model=MODEL,instructions=instructions,input=items).output_text

@app.get("/")
async def home(): return FileResponse(os.path.join(ROOT,"static","index.html"))

@app.get("/api/health")
async def health(): return {"ok":True,"version":"9.0"}

@app.get("/api/status")
async def status():
 return {"configured":bool(API_KEY),"model":MODEL if API_KEY else "demo","provider":"OpenAI Responses API" if API_KEY else "Demo","memory":True}

@app.get("/api/settings")
async def settings(): return {"assistant_name":ASSISTANT_NAME,"version":"9.0","model":MODEL}
...
async def add_memory(req:MemoryRequest):
 ...
 c=db()
 try:
  c.execute("INSERT INTO memories(fact,created_at) VALUES(?,?)",(fact,datetime.utcnow().isoformat()));c.commit()
 except sqlite3.IntegrityError: pass
 c.close();return {"ok":True,"fact":fact}

@app.delete("/api/memory/{memory_id}")
async def delete_memory(memory_id:int):
 c=db();c.execute("DELETE FROM memories WHERE id=?",(memory_id,));c.commit();c.close()
 return {"ok":True}
...
async def chat(req:ChatRequest):
 ...
 hist=[(x["role"],x["content"]) for x in old]
 facts=[x["fact"] for x in memories]
 try: reply=await real_answer(m,hist,facts) if API_KEY else demo_answer(m)
 except Exception as e: reply=f"AI connection error: {e}"
 now=datetime.utcnow().isoformat()
 c.execute("INSERT INTO messages(role,content,created_at) VALUES(?,?,?)",("user",m,now))
 c.execute("INSERT INTO messages(role,content,created_at) VALUES(?,?,?)",("assistant",reply,datetime.utcnow().isoformat()))
 c.commit();c.close()
